    ...
    <div class="mb-8">
        ...
        <form action="{{ route('search') }}" method="GET" role="search" class="flex gap-3 max-w-xl">
            <input type="search" name="q" value="{{ $query }}" placeholder="Search posts…" autofocus
                   class="flex-1 rounded-lg border border-gray-200 bg-white text-gray-900 placeholder-gray-400 px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-transparent">
            <button type="submit"
                    class="rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2 text-sm font-medium transition-colors">
                Search
            </button>
        </form>
    </div>
    ...
    @if ($query)
        @if ($posts->isEmpty())
            <div class="py-16 text-center">
                <svg class="size-12 mx-auto mb-4 text-gray-200" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-4.35-4.35M17 11A6 6 0 115 11a6 6 0 0112 0z"/></svg>
                <p class="text-lg font-semibold mb-1">No results for &ldquo;{{ $query }}&rdquo;</p>
                <p class="text-sm text-gray-400 mb-6">Try different keywords or browse all posts.</p>
                <a href="{{ route('posts.index') }}" class="text-sm font-medium text-indigo-600 hover:underline">Browse all posts &rarr;</a>
            </div>
        @else
            <p class="text-sm text-gray-400 mb-6">
                {{ $posts->total() }} result{{ $posts->total() === 1 ? '' : 's' }} for <strong class="text-gray-700">{{ $query }}</strong>
            </p>

            <div class="flex flex-col gap-4">
                @foreach ($posts as $post)
                    <article class="bg-white rounded-xl border border-gray-200 hover:shadow-md transition-shadow flex gap-5 p-5">
                        ...
                        <div class="flex flex-col flex-1 gap-1.5 min-w-0">
                            ...
                            <div class="mt-auto flex items-center gap-3 text-xs text-gray-400">
                                ...
                                @if ($post->author)
                                    <a href="{{ route('author.show', $post->author->slug) }}" class="hover:text-indigo-600 transition-colors">
