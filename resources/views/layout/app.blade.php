...
<body class="bg-gray-50 text-gray-900 min-h-screen flex flex-col">
    ...
    <header class="bg-white border-b border-gray-200 sticky top-0 z-10">
        <div class="max-w-8xl mx-auto px-18 py-4 flex items-center justify-between gap-4">
            <a href="{{ route('home') }}" class="text-xl font-bold tracking-tight text-gray-900 hover:text-indigo-600 transition-colors shrink-0">
                Blog
            </a>
            <form action="{{ route('search') }}" method="GET" role="search" class="flex-1 max-w-sm">
                <div class="relative">
                    <input type="search" name="q" value="{{ request('q') }}" placeholder="Search posts…"
                           class="w-full rounded-lg border border-gray-200 bg-gray-50 text-gray-900 placeholder-gray-400 px-4 py-1.5 pr-10 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-transparent">
            ...
            <nav class="flex items-center gap-4 text-sm font-medium text-gray-600 shrink-0">
                <a href="{{ route('posts.index') }}" class="hover:text-indigo-600 transition-colors">Posts</a>
                <a href="/feed" title="RSS feed" class="p-1.5 rounded-lg text-gray-400 hover:bg-gray-100 hover:text-orange-500 transition-colors" aria-label="RSS feed">
                ...
                <button id="theme-toggle"
                    ...
                    <svg class="h-4 w-4 hidden" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <footer class="bg-white border-t border-gray-200 text-sm text-gray-500">
