{{-- Floating meta bar --}}
<div class="relative z-10 -mt-8 mb-10 bg-white dark:bg-gray-900 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-800 px-6 py-3.5 flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-gray-500 dark:text-gray-400">
    {{-- Read time --}}
    ...
    <span class="hidden sm:block text-gray-200 dark:text-gray-700 select-none">|</span>
        ...
        <span class="flex flex-wrap gap-2 shrink-0">
            @foreach (($post->terms['category'] ?? []) as $catSlug => $catName)
                <a href="{{ route('category.show', $catSlug) }}"
                   class="text-indigo-600 dark:text-indigo-400 hover:underline font-medium">{{ $catName }}</a>
            @endforeach
        </span>
        <span class="hidden sm:block text-gray-200 dark:text-gray-700 select-none">|</span>
    @endif
    ...
    {{-- Comments --}}
    @if ($commentCount > 0)
        <a href="#comments" class="flex items-center gap-1.5 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors shrink-0">
    ...
    @else
        <a href="#comments" class="hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors shrink-0">No comments</a>
    @endif
    <span class="hidden sm:block text-gray-200 dark:text-gray-700 select-none">|</span>
    ...
    <span class="flex items-center gap-2 shrink-0">
        <span class="text-xs font-medium text-gray-400 dark:text-gray-500">Share:</span>
        <a href="https://twitter.com/intent/tweet?url={{ $shareUrl }}&text={{ $shareTitle }}"
           target="_blank" rel="noopener noreferrer"
           class="p-1.5 rounded-full bg-gray-100 dark:bg-gray-800 hover:bg-black hover:text-white dark:hover:bg-white dark:hover:text-black transition-colors">
        ...
        <a href="https://www.linkedin.com/sharing/share-offsite/?url={{ $shareUrl }}"
           target="_blank" rel="noopener noreferrer"
           class="p-1.5 rounded-full bg-gray-100 dark:bg-gray-800 hover:bg-[#0077B5] hover:text-white transition-colors">
        ...
        <button type="button" onclick="copyShareLink(this, '{{ route('posts.show', $post->slug) }}')"
                class="p-1.5 rounded-full bg-gray-100 dark:bg-gray-800 hover:bg-indigo-600 hover:text-white transition-colors" aria-label="Copy link">
